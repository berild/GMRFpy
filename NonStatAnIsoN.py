...
class NonStatAnIso:
    ...
    #         H = vv[:,:,:,np.newaxis]*dv[:,:,np.newaxis,:]  + dv[:,:,:,np.newaxis]*vv[:,:,np.newaxis,:] + dw[:,:,:,np.newaxis]*ww[:,:,np.newaxis,:] + ww[:,:,:,np.newaxis]*dw[:,:,np.newaxis,:]
    #     elif var == 2: #vy
    #         dpar = np.zeros(27)
    #         dpar[d] = 1
    #         vv = np.stack([self.grid.evalBH(par = vx),self.grid.evalBH(par = vy),self.grid.evalBH(par = vz)],axis=2)
    ...
    #         H = ww[:,:,:,np.newaxis]*dw[:,:,np.newaxis,:] + dw[:,:,:,np.newaxis]*ww[:,:,np.newaxis,:]
    #     elif var == 5: #rho2
    #         dpar = np.zeros(27)
    #         dpar[d] = 1
    #         vv = np.stack([self.grid.evalBH(par = vx),self.grid.evalBH(par = vy),self.grid.evalBH(par = vz)],axis=2)
        ...
        if self.grad:
            ...
            if self.verbose:
                if self.truth is not None:
                    ...
                    "vz = %2.2f"%(np.mean(np.abs(par[108:135])-np.abs(self.truth[108:135]))),"\u03C1_1 = %2.2f"%(np.mean(par[135:162]-self.truth[135:162])),"\u03C1_2 = %2.2f"%(np.mean(par[162:189]-self.truth[162:189])), "\u03C3 = %2.2f"%(np.mean(par[189]-self.truth[189])))
                else:  
