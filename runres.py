    ...
    for i in range(3): #dho
        for j in range(3): #r
            if model == 4:
                ...
                res[1][:54,(i)*3 + j] = np.mean((pars[np.where(((pars[:,npars]==(i+1))&(pars[:,npars+1]==(j+1)))),:npars][0,:,:54] - truth[:54][np.newaxis,:])**2,axis = 0)
                ...
                res[1][2:7,(i)*3 + j] = np.mean((np.abs(pars[np.where(((pars[:,npars]==(i+1))&(pars[:,npars+1]==(j+1)))),:npars][0,:,2:7]) - np.abs(truth[2:7])[np.newaxis,:])**2,axis = 0)
