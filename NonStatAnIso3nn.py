...
class NonStatAnIso:
    ...
    def getH(self,gamma = None,vx = None,vy = None, vz = None,rho1 = None,rho2 = None,d=None,var = None):
        ...
        if var is None:
            ...
            vb1 = np.stack([-vv[:,:,1],vv[:,:,0],vv[:,:,2]*0],axis=2)
            dvb1 = np.stack([-dv[:,:,1],dv[:,:,0],dv[:,:,2]*0],axis=2)
            vb2 = np.stack([-vv[:,:,2]*vv[:,:,0],-vv[:,:,2]*vv[:,:,1],vv[:,:,0]**2 + vv[:,:,1]**2],axis=2)
            dvb2 = np.stack([-vv[:,:,2]*dv[:,:,0],-vv[:,:,2]*dv[:,:,1],2*vv[:,:,0]*dv[:,:,0]],axis=2)
            ww = vb1*self.grid.evalBH(par = rho1)[:,:,np.newaxis] + vb2*self.grid.evalBH(par = rho2)[:,:,np.newaxis]
            ...
            vb1 = np.stack([-vv[:,:,1],vv[:,:,0],vv[:,:,2]*0],axis=2)
            dvb1 = np.stack([-dv[:,:,1],dv[:,:,0],vv[:,:,2]*0],axis=2)
            vb2 = np.stack([-vv[:,:,2]*vv[:,:,0],-vv[:,:,2]*vv[:,:,1],vv[:,:,0]**2 + vv[:,:,1]**2],axis=2)
            dvb2 = np.stack([-vv[:,:,2]*dv[:,:,0],-vv[:,:,2]*dv[:,:,1],2*vv[:,:,1]*dv[:,:,1]],axis=2)
            ww = vb1*self.grid.evalBH(par = rho1)[:,:,np.newaxis] + vb2*self.grid.evalBH(par = rho2)[:,:,np.newaxis]
            dw = dvb1*self.grid.evalBH(par = rho1)[:,:,np.newaxis] + dvb2*self.grid.evalBH(par = rho2)[:,:,np.newaxis]
            H = 2*dv[:,:,:,np.newaxis]*vv[:,:,np.newaxis,:] + 2*dw[:,:,:,np.newaxis]*ww[:,:,np.newaxis,:]
        elif var == 3: #vz
            ...
            dvb1 = np.stack([-dv[:,:,1],dv[:,:,0],vv[:,:,2]*0],axis=2)
            vb2 = np.stack([-vv[:,:,2]*vv[:,:,0],-vv[:,:,2]*vv[:,:,1],vv[:,:,0]**2 + vv[:,:,1]**2],axis=2)
            dvb2 = np.stack([-vv[:,:,0]*dv[:,:,2],-vv[:,:,1]*dv[:,:,2],vv[:,:,1]*0],axis=2)
            ww = vb1*self.grid.evalBH(par = rho1)[:,:,np.newaxis] + vb2*self.grid.evalBH(par = rho2)[:,:,np.newaxis]
            dw = dvb1*self.grid.evalBH(par = rho1)[:,:,np.newaxis] + dvb2*self.grid.evalBH(par = rho2)[:,:,np.newaxis]
            H = 2*dv[:,:,:,np.newaxis]*vv[:,:,np.newaxis,:] + 2*dw[:,:,:,np.newaxis]*ww[:,:,np.newaxis,:]
        elif var == 4: #rho1
